...
double dnpir2N (double phi, void *para1)    
{
  ...
  double temp=0;
  temp=dnr * jac * jac / (2.0 * sume * sume);
...
void add_reso (pn, pnR, k, j)
     long int pn;			/* internal number of daughter part. */
     long int pnR;			/* internal number of Resonance */
     long int k;			/* daughter-particle place in the decay
				   array  */
     long int j;			/* internal number of decay */
...

{
  ...
  long int pn2, pn3, pn4;		/* internal numbers for resonances */
  long int part;
  int l, i;
  int npt, nphi;
	...
	for (l = 0; l < npt; l++)
	  {
		...
		if (isnan(particle[pn].dNdptdphi[l][i])!=0)
		  {
		    printf("Problem in 2-body, pn=%i, pnR=%i l=%i i=%i\n",pn,pnR,l,i);
		    printf("pt=%f phi=%f m1=%f m2=%f mr=%f monv=%i\n",particle[pn].pt[l], PHI[i],m1,m2,mr,particle[pnR].monval);
		  }
	      }
	  }
	break;
	...
	for (i = 0; i < 4*nphi; i++)
	  {
		...
		if (isnan(particle[pn].dNdptdphi[l][i])!=0)
		  printf("Problem in 3-body, pn=%i, pnR=%i l=%i i=%i\n",pn,pnR,l,i);
	      }
	  }
	break;
      }
	...
	for (i = 0; i < 4*nphi; i++)
	  {
		...
		particle[pn].dNdptdphi[l][i] += decay[j].branch *
		  Edndp3_3bodyN (y, particle[pn].pt[l], PHI[i],
				 m1, m2, m3, mr, norm3, particle[pnR].monval);
		if (isnan(particle[pn].dNdptdphi[l][i])!=0)
		  printf("Problem in 4-body, pn=%i, pnR=%i l=%i i=%i\n",pn,pnR,l,i);
